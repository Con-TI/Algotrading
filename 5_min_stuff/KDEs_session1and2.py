...
# reference = f'{date} 12:00:00+07:00'

pp = PdfPages('../pdfs/5_min_data(1_month)/KDE_of_high_low_spreads_in_session1and2.pdf')
...
for stock in filtered_codes:
    ...
    for daily_dataframe in daily_dataframes:
        date = daily_dataframe.index[0].strftime('%Y-%m-%d')
        reference = pd.to_datetime(f'{date} 12:00:00+07:00')
        session_1 = daily_dataframe[daily_dataframe.index <= reference]
        session_2 = daily_dataframe[daily_dataframe.index > reference]
        low_1 = session_1['Low'][stock]
        high_1 = session_1['High'][stock]
        session_1_high_low_spread = (high_1.values.max()-low_1.values.min())/low_1.values.min()*100
        session_1_spreads.append(session_1_high_low_spread)
        low_2 = session_2['Low'][stock]
        high_2 = session_2['High'][stock]
        session_2_high_low_spread = (high_2.values.max()-low_2.values.min())/low_2.values.min()*100
        session_2_spreads.append(session_2_high_low_spread)
    spreads_session_1 = figure.add_subplot(grid[0,0])
    sns.kdeplot(session_1_spreads,ax=spreads_session_1)
    spreads_session_1.set_title(f'Spreads of session 1 for {stock}')
    spreads_session_1.axvline(np.mean(session_1_spreads)+np.std(session_1_spreads), color='g', linestyle='--', label=round(np.mean(session_1_spreads)+np.std(session_1_spreads),3))
    spreads_session_1.axvline(np.mean(session_1_spreads)-np.std(session_1_spreads), color='r', linestyle='--', label=round(np.mean(session_1_spreads)-np.std(session_1_spreads),3))
    spreads_session_1.legend()

    spreads_session_2 = figure.add_subplot(grid[0,1])
    sns.kdeplot(session_2_spreads,ax=spreads_session_2)
    spreads_session_2.set_title(f'Spreads of session 2 for {stock}')
    spreads_session_2.axvline(np.mean(session_2_spreads)+np.std(session_2_spreads), color='g', linestyle='--', label=round(np.mean(session_2_spreads)+np.std(session_2_spreads),3))
    spreads_session_2.axvline(np.mean(session_2_spreads)-np.std(session_2_spreads), color='r', linestyle='--', label=round(np.mean(session_2_spreads)-np.std(session_2_spreads),3))
    spreads_session_2.legend()
    pp.savefig()
    # plt.show()
    plt.close()
pp.close()
