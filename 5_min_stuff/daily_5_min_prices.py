...
# for n in range(0,1088,67):
#     print(filtered_data[n:n+1])

close_prices = filtered_data['Close'][filtered_codes]
open_prices = filtered_data['Open'][filtered_codes]
high_prices = filtered_data['High'][filtered_codes]
low_prices = filtered_data['Low'][filtered_codes]
...
pp = PdfPages('../pdfs/5_min_data(1_month)/5_min_daily_price_charts_of_stocks_with_high_low_spread.pdf')

for stock in filtered_codes:
    length_of_list = len(daily_dataframes)
    pair = find_factor_pair(length_of_list)
    print(pair)
    fig, axs = plt.subplots(nrows=pair[0], ncols=pair[1], figsize=(13, 7), num=stock, )
    n=-1
    for daily_dataframe in daily_dataframes:
        n+=1
        close_prices = daily_dataframe['Close'][stock]
        open_prices = daily_dataframe['Open'][stock]
        high_prices = daily_dataframe['High'][stock]
        low_prices = daily_dataframe['Low'][stock]
        low_of_day = low_prices.values.min()
        high_of_day = high_prices.values.max()
        spread = (high_of_day-low_of_day)/low_of_day*100
        series_s = [close_prices,open_prices,high_prices,low_prices]
        column = int(n % pair[1])
        row = int(math.floor(n/pair[1]))
        for series in series_s:
            axs[row,column].plot(series.index,series.values)
        axs[row,column].axhline(low_of_day, color='k', linestyle='--')
        axs[row,column].axhline(high_of_day, color='k', linestyle='--')
        axs[row, column].set_xticklabels([])
        axs[row,column].set_title(f'{round(spread,2)}', fontsize=label_fontsize)
